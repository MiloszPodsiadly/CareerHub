    ...
    public class JobOfferCommandService {
        ...
        private static String generatedExternalId(JobSource source) {
            return source.name().toLowerCase() + "-" + UUID.randomUUID();
        }

        @Transactional
        public JobOfferDetailDto create(User owner, JobOfferCreateRequest req, String platformBaseUrl) {
            JobSource source = JobSource.PLATFORM;
            String externalId = generatedExternalId(source);
        ...
        public JobOfferDetailDto updateOwned(User currentUser, Long offerId, JobOfferUpdateRequest req) {
            ...
            if (e.getSource() != JobSource.PLATFORM) {
                throw new IllegalStateException("Only platform offers can be modified.");
            }
        ...
        public void deleteOwned(User currentUser, Long offerId, String plainPassword) {
            ...
            if (offer.getSource() != JobSource.PLATFORM) {
                throw new IllegalStateException("Only platform offers can be deleted.");
            }
