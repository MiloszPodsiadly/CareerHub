            ...
            if (ch.nodeType === 1) {
                ...
                if (tag === 'a') {
                    [...ch.attributes].forEach((a) => {
                        if (a.name.toLowerCase() !== 'href') ch.removeAttribute(a.name);
                    });
...
function platformRouteFromUrl(url, numericId) {
...
export function initJobs(opts = {}) {
    ...
    const $list = byId('list'),
        ...
        $q = byId('q'),
        $city = byId('city'),
        $seniority = byId('seniority'),
        $contract = byId('contract'),
        $sort = byId('sort'),
        $group = byId('group'),
        $withSalary = byId('withSalary'),
        $remote = byId('remote'),
        $reset = byId('reset');
            ...
            keywords: Array.isArray(x.techTags)
                ...
                : Array.isArray(x.keywords)
                    ...
                    : Array.isArray(x.techStack)
                        ? x.techStack.map((s) => s?.name).filter(Boolean)
                        : [],
            salary: toSalary(x),
            remote: !!x.remote,
    ...
    function groupKey(job) {
        return state.filters.group === 'city' ? job.city || 'Other' : job.company || 'Company';
    }
    ...
    function renderCard(job) {
        ...
        const applyBtn = internal
            ...
            : job.url
                ? `<a class="chip" href="${job.url}" target="_blank" rel="noopener">Apply ↗</a>`
                : '';
    ...
    function openModal(job) {
        ...
        const applyCta = internal
            ...
            : job.url
                ? `<a class="chip chip--apply" href="${job.url}" target="_blank" rel="noopener">Apply ↗</a>`
                : '';
    ...
    $city?.addEventListener('input', (e) => {
        state.filters.city = e.target.value.trim();
        deb(() => gotoPage(1));
    });
    $seniority?.addEventListener('change', (e) => {
        state.filters.seniority = e.target.value;
        gotoPage(1);
    });
...
const DEMO_ITEMS = [
    {
        ...
        country: 'PL',
        description:
            'Microservices, Spring Boot, Kafka, AWS.\n\n- Build APIs\n- Own CI/CD\n- Work with Kafka\n\nRequirements:\n1. Java 17+\n2. Spring Boot\n3. AWS',
    ...
    },
    {
        ...
        city: 'Krakow',
        country: 'PL',
        description: 'React, TypeScript, Vite, REST.',
        salary: { currency: 'PLN', min: 18000, max: 25000, period: 'MONTH' },
        datePosted: '2025-09-28T09:00:00Z',
        keywords: ['React', 'TypeScript', 'Vite', 'REST'],
        contract: 'UOP',
        contracts: ['UOP'],
        level: 'MID',
    },
