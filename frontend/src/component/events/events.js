...
const MOCK = (() => {
    ...
    for (let i=0;i<200;i++){
        ...
        items.push({ id:`mock-${i}`, source:"mock", title, url, city, country, startAt: start.toISOString(), categories: [type, tech] });
    }
    for (let i=0;i<20;i++){
...
export function initEvents(){
    ...
    $("#country").addEventListener("change", () => {
        ...
        const disableCity = !val || val === "" || val === "ZZ-ONLINE";
        $("#city").disabled = disableCity;
        if (disableCity) $("#city").value = "";
        onChange();
    });
    ...
    $("#city").disabled = !$("#country").value || $("#country").value === "ZZ-ONLINE";
    onChange();
}
...
function filterCountry(query){
    ...
    if (![...sel.options].some(o => o.value === state.country)) {
        ...
        $("#city").disabled = (state.country === "" || state.country === "ZZ-ONLINE");
    }
}
...
async function fetchAndRender(){
    ...
    try{
        const res = MODE === 'mock' ? MOCK.search(params) : await fetchLive(params);
        ...
        if (!container.querySelector(".error")) container.insertAdjacentHTML("afterbegin", `<div class="error">Load error: ${escapeHtml(e.message)}</div>`);
    }
}
...
function renderCards(items){
    ...
    <article class="card">
      ...
      <div class="where">${ev.city ? escapeHtml(ev.city)+", " : ""}${escapeHtml(ev.country || "—")} • ${escapeHtml(ev.date)}</div>
...
function setStatus(txt){
    const s = $("#status");
    if(!txt){ s.hidden=true; s.textContent=""; } else { s.hidden=false; s.textContent=txt; }
}
