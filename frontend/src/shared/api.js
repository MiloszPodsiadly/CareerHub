...
async function request(path, opts={}, retry=true){
    ...
    if (res.status === 401 && retry && !isAuthPath(path)) {
        try { await doRefresh(); return request(path, opts, false); } catch {}
    }
...
function emitReady(){ window.dispatchEvent(new CustomEvent('auth:ready', { detail: getUser() })); }
...
export async function bootstrapAuth(){
    if (!getAccess()) { setUser(null); emitReady(); return null; }
